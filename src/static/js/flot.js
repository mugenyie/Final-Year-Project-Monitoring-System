...
c=$.color.parse(options.colors[i]);var sign=variation%2==1?-1:1;c.scale('rgb',1+sign*Math.ceil(variation/2)*0.2);colors.push(c);++i;if(i>=options.colors.length){i=0;++variation;}}
...
axis.datamax=null;});}
function backingScale(cctx){if(window.devicePixelRatio>1&&(cctx.webkitBackingStorePixelRatio===undefined||cctx.webkitBackingStorePixelRatio<2)){return window.devicePixelRatio;}
return 1;}
function makeCanvas(skipPositioning,cls){var c=document.createElement('canvas');c.className=cls;var cctx=c.getContext("2d");canvasBackingScale=backingScale(cctx);c.width=canvasBackingScale*canvasWidth;c.height=canvasBackingScale*canvasHeight;c.style.width=canvasWidth+"px";c.style.height=canvasHeight+"px";if(!skipPositioning)
$(c).css({position:'absolute',left:0,top:0});$(c).appendTo(placeholder);if(!c.getContext)
c=window.G_vmlCanvasManager.initElement(c);cctx.save();cctx.scale(canvasBackingScale,canvasBackingScale);return c;}
...
function resizeCanvas(c){var cctx=c.getContext("2d");canvasBackingScale=backingScale(cctx);if(c.style.width!=canvasWidth){c.width=canvasBackingScale*canvasWidth;c.style.width=canvasWidth+"px";}
if(c.style.height!=canvasHeight){c.height=canvasBackingScale*canvasHeight;c.style.height=canvasHeight+"px";}
cctx.restore();cctx.save();cctx.scale(canvasBackingScale,canvasBackingScale);}
...
axisMargin=0;if(tickLength==null){var sameDirection=$.grep(all,function(a){return a&&a.reserveSpace;});var innermost=$.inArray(axis,sameDirection)==0;if(innermost)
...
plotOffset[a]+=showGrid?options.grid.borderWidth:0;$.each(axes,function(_,axis){axis.show=axis.options.show;if(axis.show==null)
...
noTicks=0.3*Math.sqrt(axis.direction=="x"?canvasWidth:canvasHeight);axis.delta=(axis.max-axis.min)/noTicks;if(!opts.mode){var maxDec=opts.tickDecimals;var dec=-Math.floor(Math.log(axis.delta)/Math.LN10);if(maxDec!=null&&dec>maxDec)
...
size=opts.minTickSize;axis.tickDecimals=Math.max(0,maxDec!=null?maxDec:dec);axis.tickSize=opts.tickSize||size;axis.tickGenerator=function(axis){var ticks=[];var start=floorInBase(axis.min,axis.tickSize),i=0,v=Number.NaN,prev;do{prev=v;v=start+i*axis.tickSize;ticks.push(v);++i;}while(v<axis.max&&v!=prev);return ticks;};axis.tickFormatter=function(v,axis){return v.toFixed(axis.tickDecimals);};}
...
if(lf)
label=lf(label,s);fragments.push('<td class="legendColorBox"><div style="border:1px solid '+options.legend.labelBoxBorderColor+';padding:1px"><div style="width:4px;height:0;border:5px solid '+s.color+';overflow:hidden"></div></div></td>'+
...
return gradient;}}}
$.plot=function(placeholder,data,options){var plot=new Plot($(placeholder),data,options,$.plot.plugins);return plot;};$.plot.version="0.7";$.plot.plugins=[];function floorInBase(n,base){return base*Math.floor(n/base);}})(jQuery);